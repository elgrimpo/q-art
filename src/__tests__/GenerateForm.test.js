...
describe('GenerateForm', () => {
  ...
  // ---- 11. Transient poll failure is retried transparently ----
  test('retries a transient poll failure before eventually succeeding', async () => {
    mockStartGeneration.mockResolvedValueOnce({ job_id: 'job-6' })
    mockGetGenerationProgress
  ...
  // ---- 12. Gives up after repeated poll failures exceed the retry allowance ----
  test('gives up after repeated poll failures exceed the retry allowance', async () => {
    mockStartGeneration.mockResolvedValueOnce({ job_id: 'job-7' })
    ...
    // 4 consecutive failures exhausts the "up to 3 retries" allowance.
  ...
  // ---- 13. Gives up after the ~2-minute overall cap ----
  test('gives up after ~2 minutes without a terminal status', async () => {
    mockStartGeneration.mockResolvedValueOnce({ job_id: 'job-8' })
    mockGetGenerationProgress.mockResolvedValue({ status: 'processing', percent: 10, stage: 'novita', eta: null })
    ...
    // Advance the mocked clock past the 2-minute cap; the next real poll tick
    // (~1.2s later) will see Date.now() - startedAt > 120000 and give up.
    now += 121000
